...
func (c *Controller) AddPod(ctx context.Context, req *api.AddPodRequest) error {
	...
	return c.createEntity(ctx, req.ID, topoapi.PodKind, nil, map[string]string{topoapi.PodKind: req.ID})
}
...
func (c *Controller) AddRack(ctx context.Context, req *api.AddRackRequest) error {
	...
	if err := c.createEntity(ctx, req.ID, topoapi.RackKind, nil, labels(req.PodID, req.ID)); err != nil {
		return err
	}
	return c.createRelation(ctx, req.PodID, req.ID, topoapi.CONTAINS)
}
...
func (c *Controller) AddSwitch(ctx context.Context, req *api.AddSwitchRequest) error {
	...
	if err := c.createEntity(ctx, req.ID, topoapi.SwitchKind, al, labels(req.PodID, req.RackID)); err != nil {
		return err
	}
	return c.createRelation(ctx, req.RackID, req.ID, topoapi.CONTAINS)
}
...
func (c *Controller) AddServerIPU(ctx context.Context, req *api.AddServerIPURequest) error {
	...
	if err := c.createEntity(ctx, req.ID, topoapi.ServerKind, nil, labels(req.PodID, req.RackID)); err != nil {
		return err
	}
	if err := c.createRelation(ctx, req.RackID, req.ID, topoapi.CONTAINS); err != nil {
		return err
	}

	ipuID := fmt.Sprintf("%s-IPU", req.ID)
	al := aspects(req.ChassisConfigID, req.PipelineConfigID, req.GNMIEndpoint, req.P4Endpoint)
	if err := c.createEntity(ctx, ipuID, topoapi.IPUKind, al, labels(req.PodID, req.RackID)); err != nil {
		return err
	}
	return c.createRelation(ctx, req.ID, ipuID, topoapi.CONTAINS)
}
...
func endpoint(ep string) *topoapi.Endpoint {
	...
	return &topoapi.Endpoint{Address: fields[0], Port: uint32(port)}
}

// Produces a map of pod and rack labels
func labels(pod string, rack string) map[string]string {
	return map[string]string{topoapi.PodKind: pod, topoapi.RackKind: rack}
}
...
func (c *Controller) createEntity(ctx context.Context, id string, kindID string, aspectList []*types.Any, labels map[string]string) error {
	...
	_, err := c.topoClient.Create(ctx, &topoapi.CreateRequest{
